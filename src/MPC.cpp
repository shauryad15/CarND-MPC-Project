...
Model MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  size_t i;
  ...
  Model model;
  model.steer = solution.x[delta_start];
  model.throttle = solution.x[a_start];

  //rest all contains the x and y coordinates
  for(i=0; i<N; i++)
  {
    model.mpc_x_vals.push_back(solution.x[x_start + i]);
    model.mpc_y_vals.push_back(solution.x[y_start + i]);
  }
  return model;
}
